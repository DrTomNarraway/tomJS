...
const Trials = ((module) => {
	...
	module.BratzkeSignal = class BratzkeSignal extends module.Trial {

		constructor(args={}) {

			if (!('condition' in args))
				tomJS.error('no condition passed to Bratzke signal trial');

			super(args);

			this.data.signal_after = Number(args.signal_after ?? 2000);
			this.data.signal_duration = Number(args.signal_duration ?? 300);
			...
			this.signal = new Stimuli.BratzkeBars(this, args);

			this.data.stimulus_duration += this.data.condition + this.data.signal_duration;
			this.data.bar_duration = this.data.fixation_duration
				+ this.data.condition
				+ this.data.signal_duration;
		...
		enter() {
			...
			this.data.signal_off = this.data.signal_on + this.data.signal_duration;
			this.data.early = this.data.signal_on - this.data.stimulus_fast;
			this.data.late = this.data.signal_off + this.data.stimulus_slow;
			this.data.window_width = this.data.signal_duration / this.data.bar_duration;
		}
		...
		updateBars() {
			if (this.timeline.currentState() == "Feedback" |
				this.timeline.currentState() == "ITI") return;
			this.data.percent = (tomJS.now - this.data.fixation_on) / this.data.bar_duration;
			this.changeColours();
		}

		changeColours() {
			if (tomJS.now > this.data.late) {
				this.data.note_colour = "#ffffff00";
				this.data.window_colour = "white";
			}
			else if (tomJS.now > this.data.early) {
				this.data.note_colour = "DodgerBlue";
				this.data.window_colour = "DodgerBlue";
			} 
			else {
				this.data.note_colour = "white";
				this.data.window_colour = "white";
			}
		}
