...
class Experiment {	
	...
	constructor(args={}) {
		...
		this.debug.inputs     = args.inputs     ?? false;
	...
	drawInputs() {
		if (Object.keys(this.keyboard.keys).length == 0) return;
		for (let i = 0; i < Object.keys(this.keyboard.keys).length; i++) {
			const k = Object.keys(this.keyboard.keys)[i];
			const v = Object.values(this.keyboard.keys)[i];
			const y = 0.025 + (i * 0.05);
			this.writeToCanvas(k + " " + v, {'y':y, 'x':0.05, 'colour':'yellow'});
		};
	}
	...
	update () {				
		...
		if (this.debug.inputs) this.drawInputs();
	}
...
class Trial extends State {
	...
	constructor(args={}) {
		...
		if ('timeline' in args) this.timeline = args.timeline
			...
			const _fixation = new Fixation(this, args);
			...
			const _blank = new Stimulus(this, args);
...
class Slide extends State {
	...
	evaluate(operation) {
		const t = operation.split(' ');
		const l = this.parseText(t[0]);
		const o = t[1];
		const r = this.parseText(t[2])
		switch (o) {
			case '==': return l == r;
			case '!=': return l != r;
			case '<':  return l <  r;
	...
	checkConditions(content) {
		...
		for (let c of content.conditions) if (!this.evaluate(c)) return false;
		return true;
	}
	...
	parseText(_text) {
		let _out = _text;
		while (_out.includes("~")) {
			const l = _out.indexOf("~");
			const r = _out.indexOf("~", l + 1);
			const before = _out.substring(0, l);
			const middle = _out.substring(l + 1, r);
			const after = _out.substring(r + 1);
			_out = before + eval(middle) + after;
		};
		return _out;
	}
		...
		for (let c of this.content) {
			switch(c.class) {
				...
				case 'table':
					...
					break;
				case 'progressbar':
					this.bar_start = this.data.start;
...
class Consent extends Slide {
	...
	update() {
		...
		if (tomJS.now < this.data.start + this.force_wait) return;
	}
...
class Countdown extends Slide {
	...
	update() {
		let time = Math.ceil((this.data.start + this.lifetime - tomJS.now) / 1000);
        tomJS.writeToCanvas(time, {'fontSize':this.fontSize});
        if (tomJS.now >= this.data.start + this.lifetime) this.complete = true;
		super.update();
	}
...
class Text extends Stimulus {
	...
	calculateFontSize() {
		this.data.fontSize = Math.round(this.data.size * tomJS.visual.stimulus_size) + "px";
	}

}


class Fixation extends Text {
